...
const FORBIDDEN = Object.freeze(
  Object.assign(Object.create(null), {
    ...
    // @rello-platform/slugs. Highest false-positive risk because these
    // collide with common English words (scout, drumbeat, oven, newsletter,
    // pathfinder), Scout's own `ToolSlug` namespace (newsletter), HH's
    // `PipelineStep` state-machine (newsletter), and UTM display labels
    // (oven, newsletter). For these forms, v0.2.0 narrows further: fire ONLY
    // in case #4 (typed VariableDeclarator referencing a slug type). Cases
    // #1-3 (CallExpression / BinaryExpression / SwitchCase) are skipped on
    // short forms because AST-only context can't distinguish a real slug
    // comparison (`slug === "newsletter"`) from a state-machine comparison
    // (`currentStep === "newsletter"`). The downstream type-checker covers
    // any drift in case #1-3 short-form contexts via the slug-typed signature
    // on the receiving function/operand.
    scout: "home-scout",
...
// Short-form aliases — only fire in case #4 (typed VariableDeclarator).
// See FORBIDDEN comment block above for rationale.
const SHORT_FORM_ALIASES = Object.freeze(
  Object.assign(Object.create(null), {
    scout: true,
    drumbeat: true,
    oven: true,
...
function isShortFormAlias(value) {
  return Object.prototype.hasOwnProperty.call(SHORT_FORM_ALIASES, value);
}
...
//
// `literal` is the literal's string value — short-form aliases (scout,
// drumbeat, oven, newsletter, pathfinder, the-home-scout) are restricted to
// case #4 only because they collide with English words / consumer-internal
// namespaces / state-machine values.
function isSlugContext(node, literal) {
  const parent = node.parent;
  if (!parent) return false;

  const shortForm = isShortFormAlias(literal);

  // Cases 1-3 fire on long-form aliases only (UPPERCASE-concat,
  // concatenated-lowercase, camelCase / PascalCase). All of those forms are
  // platform-app-specific shapes with negligible collision risk in
  // BinaryExpression / CallExpression / SwitchCase positions.
  ...
  // value-position-preserving containment.  Fires for both long and short
  // forms because the slug-typed annotation is an unambiguous slug-context
  // signal regardless of literal shape. Stops at the first VariableDeclarator
  // (slug-type → fire; non-slug → silent) or any non-containment node
  // (silent — outside a typed init position).
  let cursor = parent;
  while (cursor) {
...
module.exports = {
  ...
  create(context) {
    function reportLiteral(node, literal) {
      ...
      if (!isSlugContext(node, literal)) return;
      context.report({
        node,
...
module.exports.SHORT_FORM_ALIASES = SHORT_FORM_ALIASES;
